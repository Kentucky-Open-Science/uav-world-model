...
class Thruster:
    ...
    def __init__(
        ...
        # TEMP DEBUG (guarded by UAV_CTRL_DEBUG): expose the per-motor thrust_const
        # and init state so we can see what the actuator actually starts from and
        # receives each step. Off by default; the Mac has no Thruster import path.
        self._dbg = os.environ.get("UAV_CTRL_DEBUG") == "1"
        self._dbg_calls = 0
        if self._dbg:
            print(f"[THRDBG] init thrust_const_env0={[float(x) for x in self.thrust_const[0].tolist()]} "
                  f"init_rps_env0={[float(x) for x in self._init_thruster_rps[0].tolist()]} "
                  f"init_curr_thrust_env0={[float(x) for x in self.curr_thrust[0].tolist()]} "
                  f"tau_inc_env0={[float(x) for x in self.tau_inc_s[0].tolist()]} "
                  f"max_rate={float(self.max_rate[0,0])} dt={self.cfg.dt} "
                  f"thrust_r={tuple(float(x) for x in self.thrust_r.tolist())}", flush=True)
    ...
    def compute(self, control_action: MultiRotorActions) -> MultiRotorActions:
        ...
        _dbg_before = ([float(x) for x in self.curr_thrust[0].cpu().tolist()]
                       if (self._dbg and self._dbg_calls < 10) else None)

        # PASSTHROUGH: bypass the rpm-domain motor filter. The shipped
        # compute_thrust_with_rpm_time_constant integrates rpm with a near-unity gain
        # (dt/(dt+tau)), and because thrust = thrust_const * rpm^2, the sqrt/rpm
        # nonlinearity makes the THRUST trajectory overshoot the commanded thrust
        # (observed: des=2.32 N -> applied=4.83 N, ~2.2x). That overshoot saturates
        # the yaw-correction motors, spins the drone, and the saturation's net
        # over-thrust climbs it. A pure passthrough (applied == des, clamped) cannot
        # overshoot, so delivered thrust == commanded thrust exactly. Realism cost:
        # the motor first-order lag is removed -- negligible for POV world-model
        # data (rigid-body + thrust + gravity + collision dynamics are still fully
        # simulated by PhysX); only the ~50 ms motor spool is elided.
        self.curr_thrust[:] = des_thrust
        ...
        # TEMP DEBUG: des vs applied -- with the passthrough these must be EQUAL
        # (modulo the min/max clamp). Confirms the actuator no longer corrupts thrust.
        if self._dbg and self._dbg_calls < 10:
            self._dbg_calls += 1
            print(f"[THRDBG] call#{self._dbg_calls} des={[round(x,2) for x in des_thrust[0].cpu().tolist()]} "
                  f"before={[round(x,2) for x in _dbg_before]} "
                  f"applied={[round(x,2) for x in self.applied_thrust[0].cpu().tolist()]}", flush=True)
